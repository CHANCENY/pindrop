...
(() => {
  ...
  async function uploadFile(file, { bar, status, removeBtn, li }, errorsWrap, list, hiddenField) {
    ...
    try {
      for (let i = 0; i < totalChunks; i++) {
        ...
        let position;
        if (totalChunks === 1)     position = "start"; // single-chunk = treat as start+end
        else if (i === 0)          position = "start";
        else if (i === totalChunks - 1) position = "end";
        else                       position = "middle";

        await postJSON(ENDPOINTS.chunk, { data, position, uploadKey });
      ...
      if (Array.isArray(files) && files.length > 0) {
        ...
        syncHiddenField(list, hiddenField);   // ← add id to hidden field

        removeBtn.onclick = async () => {
          try {
            await sendDelete(ENDPOINTS.delete(serverFile.id));
            li.remove();
            syncHiddenField(list, hiddenField); // ← remove id from hidden field
          } catch (e) {
  ...
  function wireUploader({ drop, fileInput, errorsWrap, list, hiddenField }, config) {
    ...
    function handleFiles(files) {
      for (const file of Array.from(files)) {
        ...
        /* start upload */
        uploadFile(file, ui, errorsWrap, list, hiddenField);
      }
    }
  ...
  /* Expose for manual re-init (e.g. after dynamic DOM changes) */
  window.ChunkUploader = { init };
})();